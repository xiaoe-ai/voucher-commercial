...
-- Layer: 003C Partner controlled issuance RPC
-- Source: verified live Commercial schema snapshot 2026-09-05
-- Neutralized from live behavior:
--   * no EO- voucher code prefix
--   * no Asia/Kuala_Lumpur timezone assumption
--   * no hard-coded RM currency label
...
begin
  ...
  -- Generic, customer-neutral voucher code format.
  v_voucher_code :=
    'V-' ||
  ...
  -- Do not hard-code currency symbols or customer-specific campaign names.
...
-- Legacy entry points such as create_partner_voucher() and
-- create_partner_voucher_controlled() are intentionally NOT canonicalized here.
-- Their live definitions are tied to EVO-specific voucher types / template codes.
