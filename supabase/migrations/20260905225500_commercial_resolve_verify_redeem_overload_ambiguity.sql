-- Resolve PostgREST overload ambiguity between canonical and extended Staff RPCs.
-- Keep both capability surfaces, but require explicit branch parameters on the extended forms.

-- verify_voucher(text) remains the canonical single-argument Staff/Manager RPC.
-- verify_voucher(text,text) remains the explicit branch-aware form.
drop function if exists public.verify_voucher(text,text);

create function public.verify_voucher(p_voucher_code text, p_branch_code text)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  return jsonb_build_object(
    'success',true,'voucher_id',v_voucher.id,'voucher_code',v_voucher.voucher_code,
    'customer_name',v_voucher.customer_name,'customer_phone',v_voucher.customer_phone,
    'voucher_type',v_voucher.voucher_type,'expiry_date',v_voucher.expiry_date,
    ...
    'canonical_status',v_voucher.status,'usage_limit',1,
...
grant execute on function public.verify_voucher(text,text) to authenticated, service_role;

-- redeem_voucher(text,text) remains canonical.
-- redeem_voucher(text,text,text,text) keeps explicit branch-aware behavior; only the method keeps a default.
drop function if exists public.redeem_voucher(text,text,text,text);

create function public.redeem_voucher(p_voucher_code text, p_notes text, p_branch_code text, p_redeem_method text default 'manual_code'::text)
returns jsonb
language plpgsql
security definer
set search_path to 'public'
...
begin
  if v_method_input not in ('qr','qr_scan','manual','manual_code','admin') then return jsonb_build_object('success',false,'error','Invalid redeem method'); end if;
  v_method_store:=case when v_method_input in ('qr','qr_scan') then 'qr_scan' else 'manual' end;

  select * into v_staff from public.staff_users su where su.user_id=(select auth.uid()) and lower(coalesce(su.status,''))='active' limit 1;
  if not found then return jsonb_build_object('success',false,'error','Staff account is not authorised or is suspended'); end if;
  ...
  if lower(coalesce(v_staff.role,''))='all_branch_manager' then
    ...
    select b.id,b.branch_name into v_branch_id,v_branch_name from public.branches b where upper(b.branch_code)=upper(trim(p_branch_code)) and lower(coalesce(b.status,''))='active' limit 1;
  else
    if v_staff.branch_id is null then return jsonb_build_object('success',false,'error','Staff account has no branch assigned'); end if;
    select b.id,b.branch_name into v_branch_id,v_branch_name from public.branches b where b.id=v_staff.branch_id and lower(coalesce(b.status,''))='active' limit 1;
  end if;
  if v_branch_id is null then return jsonb_build_object('success',false,'error','Active branch not found'); end if;
  ...
  if lower(coalesce(v_voucher.status,''))='expired' or v_voucher.expiry_date < current_date then return jsonb_build_object('success',false,'error','Voucher has expired','status','expired'); end if;
  ...
  values(v_voucher.id,v_branch_id,v_staff.id,v_voucher.partner_id,v_staff.staff_name,v_method_store,'success',v_now,nullif(trim(coalesce(p_notes,'')),''))
  returning id into v_redemption_id;

  update public.vouchers set status='redeemed',redeemed_at=v_now,redeemed_by=v_staff.staff_name,usage_count=greatest(coalesce(usage_count,0),1) where id=v_voucher.id;

  return jsonb_build_object('success',true,'redemption_id',v_redemption_id,'voucher_id',v_voucher.id,'voucher_code',v_voucher.voucher_code,'customer_name',v_voucher.customer_name,'voucher_type',v_voucher.voucher_type,'branch_id',v_branch_id,'branch_name',v_branch_name,'staff_name',v_staff.staff_name,'redeemed_at',v_now,'redeem_method',v_method_store,'usage_count',1,'usage_limit',1,'remaining_uses',0,'status','redeemed');
exception when unique_violation then return jsonb_build_object('success',false,'error','Voucher has already been redeemed');
end;
$$;

revoke all on function public.redeem_voucher(text,text,text,text) from public, anon;
grant execute on function public.redeem_voucher(text,text,text,text) to authenticated, service_role;
